        ...
        while True:
            ...
            if is_before_method_passed or test.always_run:
                ...
                try:
                    ...
                    preporter.error("Failed with following message:\n%s\n%s" % (e.message, traceback.format_exc()))
                    screencapturer.take_screen_shot()
                    pconsole.write_line("%s%s|\x1b[31mFAIL\x1b[0m|" % (test_case_full_name, logger_filler))
                    ...
                    pconsole.write_line("%s%s|\x1b[32mPASS\x1b[0m|" % (test_case_full_name, logger_filler))
                    test_case.status = TestCaseStatus.PASSED
            else:
                # skip test
                preporter.warn("@%s failed, so skipped." % PDecoratorType.BeforeMethod)
                pconsole.write_line("%s%s|\x1b[33mSKIP\x1b[0m|" % (test_case_full_name, logger_filler))
